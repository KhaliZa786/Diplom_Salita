...
class chrome_locators:
    ...
    def do_registration_question(self, name, phone, mail, password):
        self.driverwait(chrome_locators.LOCATOR_USER).click()  # кликнуть на Пользователя в строке меню
        self.driverwait(chrome_locators.LOCATOR_REGISTRATION).click()  # кликнуть на Регистрация
        self.driverwait(chrome_locators.LOCATOR_USERNAME).send_keys(name)  # кликнуть на строку Имя
        self.driverwait(chrome_locators.LOCATOR_PHONE).send_keys(phone)  # кликнуть на строку Телефон
        self.driverwait(chrome_locators.LOCATOR_MAIL).send_keys(mail)  # кликнуть на строку E-mail
        self.driverwait(chrome_locators.LOCATOR_PASSWORD).send_keys(password)  # кликнуть на строку Пароль
